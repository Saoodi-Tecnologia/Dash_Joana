...
import type { DashboardMetrics, KPITrends, PerformanceTrends } from '@/types/dashboard';
...
export class AnalyticsEngine {
    ...
    async generateWeeklyInsights(): Promise<Record<string, string>> {
        try {
            ...
            const cached = localStorage.getItem(AnalyticsEngine.INSIGHTS_CACHE_KEY);
            if (cached) {
                ...
                localStorage.removeItem(AnalyticsEngine.INSIGHTS_CACHE_KEY);
            }
...
Retorne APENAS o JSON válido.`;
            ...
            localStorage.setItem(AnalyticsEngine.INSIGHTS_CACHE_KEY, JSON.stringify({
                timestamp: Date.now(),
                periodoStr,
                insights
            }));
